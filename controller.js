...
const savePatientVitals = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Patient vitals saved successfully", data: newVitals });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error saving patient vitals", error: error.message });
  }
};

// Retrieve patient vitals by patient_name
const getPatientVitals = async (req, res) => {
  try {
    const patientName = req.params.patient_name; // Get patient_name from URL params
    const vitals = await PatientVitals.find({ patient_name: patientName }).sort(
      { timestamp: -1 }
    ); // Fetch data and sort by latest first
    res
      .status(200)
      .json({ message: "Patient vitals retrieved successfully", data: vitals });
  } catch (error) {
    res
      .status(500)
      .json({
...
const getAllPatientsVitals = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error retrieving all patient vitals",
        error: error.message,
      });
  }
};
